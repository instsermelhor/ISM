...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    label: 'O Que Fazemos',
    subItems: [
      { label: 'Nossas Causas', href: '#values' },
    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        <a href="#" className="flex items-center gap-2 group z-50 relative">
          <div className="p-2 rounded-full bg-brand-600 text-white">
            <Leaf size={24} />
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-sm font-medium text-secondary-800 uppercase tracking-widest">Instituto</span>
            <span className="text-xl font-bold tracking-tight text-brand-600">Ser Melhor</span>
          </div>
        </a>
        ...
        <nav className="hidden lg:flex items-center gap-6">
          {NAV_ITEMS.map((item) => (
            ...
            >
              <a
                href={item.href || '#'}
                className="flex items-center gap-1 text-sm font-bold text-secondary-800 hover:text-brand-600 uppercase tracking-wide py-4"
              >
                {item.label}
        ...
        <nav className="flex flex-col space-y-4">
          ...
          <div className="flex flex-col gap-4 mt-8">
            <a 
              href="#donate"
              onClick={() => setIsOpen(false)}
              className="w-full flex justify-center items-center gap-2 px-6 py-4 rounded-full bg-brand-600 text-white font-bold text-sm uppercase tracking-wider shadow-md"
            >
            ...
            <a 
              href="#partner"
              onClick={() => setIsOpen(false)}
              className="w-full flex justify-center items-center gap-2 px-6 py-4 rounded-full border-2 border-brand-600 text-brand-600 font-bold text-sm uppercase tracking-wider"
            >
              <Handshake size={18} />
              Seja Parceiro
            </a>
