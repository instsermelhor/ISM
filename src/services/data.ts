...
// ── Mock Data (flat native Firestore types — SIL-ISM 1.0) ──────────────────
// These are the fallback values used in dev when Firebase is not configured.

const mockPage: InstitutionalPageAttributes = {
...
export const InstitutionalService = {
  ...
  /** Dados da página institucional — retorna InstitutionalPageAttributes plano */
  getPage: async (): Promise<InstitutionalPageAttributes> => {
    if (!FIREBASE_ENABLED) {
      return new Promise(resolve => setTimeout(() => resolve(mockPage), 300));
    }
    try {
  ...
  /** Blocos de valores/pilares — retorna ValueBlockAttributes[] */
  getValueBlocks: async (): Promise<ValueBlockAttributes[]> => {
    ...
    try {
      ...
      if (!snap.empty) {
        return snap.docs.map(d => d.data() as ValueBlockAttributes);
      }
      if (import.meta.env.DEV) {
  ...
  /** Instâncias de governança — retorna GovernanceInstanceAttributes[] */
  getGovernanceInstances: async (): Promise<GovernanceInstanceAttributes[]> => {
    ...
    try {
      ...
      const snap = await getDocs(q);
      if (!snap.empty) {
        return snap.docs.map(d => d.data() as GovernanceInstanceAttributes);
      }
      if (import.meta.env.DEV) {
  ...
  /** Marcos históricos — retorna TimelineMilestoneAttributes[] */
  getTimelineMilestones: async (): Promise<TimelineMilestoneAttributes[]> => {
    ...
    try {
      ...
      const snap = await getDocs(q);
      if (!snap.empty) {
        return snap.docs.map(d => d.data() as TimelineMilestoneAttributes);
      }
      if (import.meta.env.DEV) {
  ...
  /** Membros dos conselhos — retorna GovernanceMemberAttributes[] */
  getGovernanceMembers: async (): Promise<GovernanceMemberAttributes[]> => {
    ...
    try {
      ...
      if (!snap.empty) {
        return snap.docs.map(d => d.data() as GovernanceMemberAttributes);
      }
      if (import.meta.env.DEV) {
