...
function auditFirebaseJson(projectRoot) {
    ...
    if (data.hosting) {
        ...
        for (var _i = 0, hostings_1 = hostings; _i < hostings_1.length; _i++) {
            ...
            var siteName = hosting.site || "default-site";
            if (hosting.headers) {
                var hasCSP = false;
                var hasSTS = false;
                var hasXCTO = false;
                ...
                if (!hasCSP) {
                    findings.push({ id: generateId("FB"), title: "Missing Content-Security-Policy on ".concat(siteName), severity: "HIGH", category: "Security Headers", location: "firebase.json", description: "Missing CSP header." });
                }
                else {
                    if (!cspValue.includes("upgrade-insecure-requests")) {
                        findings.push({ id: generateId("FB"), title: "CSP missing upgrade-insecure-requests on ".concat(siteName), severity: "HIGH", category: "Transport Security", location: "firebase.json", description: "CSP should include upgrade-insecure-requests directive to prevent mixed content." });
                    }
                    if (cspValue.includes("http://")) {
                        findings.push({ id: generateId("FB"), title: "Insecure HTTP URL in CSP on ".concat(siteName), severity: "HIGH", category: "Transport Security", location: "firebase.json", description: "CSP contains unencrypted http:// origin." });
                    }
                    if (cspValue.includes("ws://")) {
                        findings.push({ id: generateId("FB"), title: "Insecure WebSocket (ws://) in CSP on ".concat(siteName), severity: "HIGH", category: "Transport Security", location: "firebase.json", description: "CSP contains unencrypted ws:// origin." });
                    }
                }
                if (!hasSTS) {
                    findings.push({ id: generateId("FB"), title: "Missing Strict-Transport-Security on ".concat(siteName), severity: "CRITICAL", category: "Transport Security", location: "firebase.json", description: "Missing Strict-Transport-Security header." });
                }
                else {
                    var maxAgeMatch = stsValue.match(/max-age=(\d+)/i);
                    var maxAge = maxAgeMatch ? parseInt(maxAgeMatch[1], 10) : 0;
                    if (maxAge < 31536000) {
                        findings.push({ id: generateId("FB"), title: "HSTS max-age insufficient on ".concat(siteName), severity: "HIGH", category: "Transport Security", location: "firebase.json", description: "HSTS max-age is ".concat(maxAge, "s, minimum required is 31536000s (1 year). Recommended is 63072000s.") });
                    }
                    if (!stsValue.toLowerCase().includes("includesubdomains")) {
                        findings.push({ id: generateId("FB"), title: "HSTS missing includeSubDomains on ".concat(siteName), severity: "HIGH", category: "Transport Security", location: "firebase.json", description: "HSTS header must include includeSubDomains." });
                    }
                    if (!stsValue.toLowerCase().includes("preload")) {
                        findings.push({ id: generateId("FB"), title: "HSTS missing preload on ".concat(siteName), severity: "MEDIUM", category: "Transport Security", location: "firebase.json", description: "HSTS header should include preload directive." });
                    }
                }
                if (!hasXCTO)
                    findings.push({ id: generateId("FB"), title: "Missing X-Content-Type-Options on ".concat(siteName), severity: "MEDIUM", category: "Security Headers", location: "firebase.json", description: "Missing X-Content-Type-Options header." });
                if (!hasXFO)
                    findings.push({ id: generateId("FB"), title: "Missing X-Frame-Options on ".concat(siteName), severity: "MEDIUM", category: "Security Headers", location: "firebase.json", description: "Missing X-Frame-Options header." });
                if (!hasRP)
                    findings.push({ id: generateId("FB"), title: "Missing Referrer-Policy on ".concat(siteName), severity: "LOW", category: "Security Headers", location: "firebase.json", description: "Missing Referrer-Policy header." });
                if (hasWildcardCORS)
                    findings.push({ id: generateId("FB"), title: "Wildcard CORS on ".concat(siteName), severity: "HIGH", category: "Security Headers", location: "firebase.json", description: "Access-Control-Allow-Origin: * found." });
            }
        }
    }
    return findings;
